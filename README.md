...
- **Embedded transcendental constants** *(shipped)* — `ln 2` and `π` are stored as
  ...
  series on every call. This speeds up `exp` (~2× at 1000 bits), `sin`/`cos`, and
  `ln` across the whole sub-65k-bit range with no global state. The deeper
  **multi-prime argument reduction** (Johansson, arXiv:2207.02501) — reduce the
  ...
  remains — needs a high-precision Babai reduction over many primes to beat the
  tuned `√`-precision method (a naive few-prime greedy reduces only a handful of
  bits); that reduction remains future work.
